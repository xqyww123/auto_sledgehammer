...
| C. Read-only dir, valid cache present | populate the cache in a writable copy first, then `chmod a-w` the dir | Build succeeds and is **fast** — store hits observable via the tracing output `[auto] store HIT` (`sledgehammer_solver.ML:1721`; `[all_auto] store HIT` at `:1812`), no switch needed. File mtime unchanged after the build (proves no write). **Warning timing:** on an all-hits theory no write site runs during the body, so the warning arrives at theory end, not at first use (see §5, "Timing") — assert that it appears, not *when*. |
